...
def extract_time_from_context(context):
    """
    Extract first HH:MM or HH:MM:SS from context string or dictionary.
    Handles cases where context might be a dict (like context_snippet).
    """
    # 1. 类型检查：如果是字典，将其转换为字符串
    #    或者你可以选择只搜索特定的字段，例如 str(context.get('Earliest Anomaly Time', ''))
    if isinstance(context, dict):
        # 策略 A: 将整个字典转为字符串进行搜索 (最简单，能覆盖所有字段)
        context_str = str(context)
        
        # 策略 B (可选优化): 如果你只想搜索特定字段，可以使用下面的代码代替上面两行
        # context_str = str(context.get('Earliest Anomaly Time', ''))
    else:
        # 如果已经是字符串，直接使用
        context_str = str(context)

    # 2. 执行正则匹配
    time_match = re.search(r'\b(\d{1,2}:\d{2}:\d{2}|\d{1,2}:\d{2})\b', context_str)

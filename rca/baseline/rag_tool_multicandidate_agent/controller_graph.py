...
def control_loop(dataset: str, objective: str, plan: str, ap, bp, logger, max_step=15, max_turn=3, rag_k=5) -> tuple:
    ...
    for cluster_id, cluster_desc in clusters:
        ...
        cluster_i_report = get_cluster_multi_grain_report(knowledge_graph_path, f"cluster_{cluster_id_num}")
...
Analyze the anomaly cluster using knowledge graph reasoning below:
- Anomaly Cluster Description: {cluster_i_report}
...
def get_cluster_multi_grain_report(json_path, cluster_id):
    ...
    if cluster_id_str in cluster_json_data["clusters"]:
        return cluster_json_data["clusters"][cluster_id_str]
    else:
        print(f"JSON 中不存在 cluster_id: {cluster_id}（对应字符串键：{cluster_id_str}）")
        print(f"可用的 cluster_id 列表：{list(cluster_json_data['clusters'].keys())}")
        return None
...
# python -m rca.baseline.rag_tool_multicandidate_agent.controller_graph
# -----------------------------

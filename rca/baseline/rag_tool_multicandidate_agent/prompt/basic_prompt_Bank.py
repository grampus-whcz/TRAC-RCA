cand = """The bank system consists of 14 pods, 6 services, and multiple nodes. Five of these services
are deployed across two pods each, while one service is deployed across four pods. This setup
provides fault tolerance; when a pod hosting a service fails, another pod with the same service can
handle the requests. The potential root cause components in this system include all 14 pods, with
eight possible causes across five categories: CPU, memory, disk, network, and JVM failures. The
telemetry data for the bank system includes metrics, traces, and logs.

cand = """The market system comprises 6 server nodes, 40 pods, and 10 services. Each service is
deployed across 4 different pods, similar to the bank system, providing fault tolerance. The potential
root cause components in this system include all 6 nodes, 40 pods, and 10 services. Notably, if all
pods hosting a particular service fail, this is considered a service-level failure rather than an individ-
ual pod failure. There are 15 possible causes of failure, spanning CPU, memory, disk, network, and
process termination. The telemetry data for the market system includes metrics, traces, and logs.

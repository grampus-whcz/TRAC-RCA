...
from main.evaluate_multi_candidate import evaluate_multi_candidate
...
def main(args, uid, dataset):
    ...
    from rca.baseline.rag_tool_multicandidate_agent.rca_agent import RCA_Agent
    import rca.baseline.rag_tool_multicandidate_agent.prompt.agent_prompt as ap
    ...
    logger.info(f"Using API_BASE: {configs['API_BASE']}")
    ...
    for idx, row in instruct_data.iterrows():
        ...
        for i in range(args.sample_num):
            ...
            logger.remove()
            logger.add(sys.stdout, colorize=False, enqueue=True, level="INFO")
            logger.add(logfile, colorize=False, enqueue=True, level="INFO")
            logger.info('\n' + "#"*80 + f"\n{uuid}: {task_index}\n" + "#"*80)
            try: 
                signal.alarm(args.timeout)

                agent = RCA_Agent(ap, bp)
                prediction, trajectory = agent.run(dataset,
                                                    instruction, 
                                                    logger, 
